...
const XMLElement* XMLNode::FirstChildElementThrow(const char* name) const
{
    for (const XMLNode* node = _firstChild; node; node = node->_next) {
        const XMLElement* element = node->ToElementWithName(name);
        if (element) {
    ...
    throw std::runtime_error(std::string("no child with name ") + name + " found");
}
...
void XMLDocument::LoadFileThrow(const char* filename)
{
    XMLError error = LoadFile(filename);
    if (error != XML_SUCCESS)
        throw std::runtime_error(ErrorName() + std::string(" ") + ErrorStr());
}
