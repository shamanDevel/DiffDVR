...
int mainWindow()
{
	...
	GLFWwindow* window = glfwCreateWindow(1420, 860, "Differentiable Volume Renderer", NULL, NULL);
	if (window == NULL)
		return 1;
...
int testTorch()
{
	std::cout << "Warp size: " << at::cuda::warp_size() << std::endl;
	std::cout << "CUDA available: " << torch::cuda::is_available() << std::endl;

	std::cout << "CUDA available: " << at::cuda::is_available() << std::endl;
	...
	std::cout << "Tensors created on the CPU" << std::endl;
	//torch::Tensor t2cuda = t2.to(c10::kCUDA);
	//td::cout << "Tensor copied to GPU" << std::endl;
	//t1 = t1.copy_(t2cuda);
