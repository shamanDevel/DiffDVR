...
def train():
    ...
    def render_images(renderer_instance, current_cameras, tf_parameterized, profiling=None):
    ...
    # reference
    reference_images, reference_tf = render_images(
            renderer, cameras,
            tf_reference.to(dtype=renderer_dtype_torch, device=device))
    ...
    class ImageLoss(torch.nn.Module):
        ...
        def forward(self, img, reference):
    ...
    with h5py.File(cfg.outputFile, 'w') as hdf5_file:
        ...
        hdf5_file.create_dataset(
            "reference_tf", data=reference_tf.cpu().numpy())
        hdf5_file.create_dataset(
        ...
        losses = dict([
            ...
            for name in image_loss.loss_names()+prior_loss.loss_names()+["total"]
        ])
        ...
        def optim_closure():
            ...
            for i in range(num_batches):
                ...
                loss_value, lx = image_loss(toCHW(images), reference_images[start:end])
                for k,v in lx.items():
                    partial_losses[k] += v
